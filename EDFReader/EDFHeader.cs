...
namespace EDFReader
{
	...
	public class EDFHeader
	{
		...
		public void CopyTo( EDFHeader header )
		{
			header.Version = this.Version;
			header.StartDateEDF = this.StartDateEDF;
			header.StartTimeEDF = this.StartTimeEDF;
			//header.NumberOfBytes = this.NumberOfBytes;
			header.isEDFPlus = this.isEDFPlus;
			header.Reserved = this.Reserved;
			header.NumberOfDataRecords = this.NumberOfDataRecords;
			header.DurationOfDataRecordInSeconds = this.DurationOfDataRecordInSeconds;
			//header.NumberOfSignalsInDataRecord = this.NumberOfSignalsInDataRecord;
			header.PatientIdentification = this.PatientIdentification;
			header.RecordingIdentification = this.RecordingIdentification;
			header.StartDateTime = this.StartDateTime;
		}

		public bool IsEDFPlus
		{
			get { return isEDFPlus; }
		}
		...
		{
			get { return _RecordingInformation; }
			set
			{
		...
		{
			get { return _NumberOfDataRecords; }
			set
			{
		...
		{
			get { return _DurationOfDataRecordInSeconds; }
			set
			{
		...
		{
			get { return _NumberOfSignalsInDataRecord; }
			set
			{
		...
		public void parseSignals( char[] signals )
		{
			...
			for( int i = 0; i < this.NumberOfSignalsInDataRecord; i++ )
			{
				...
				edf_signal.AmplifierGain = ( edf_signal.PhysicalMaximum - edf_signal.PhysicalMinimum ) / ( edf_signal.DigitalMaximum - edf_signal.DigitalMinimum );
				edf_signal.Offset = ( edf_signal.PhysicalMaximum / edf_signal.AmplifierGain ) - edf_signal.DigitalMaximum;
				edf_signal.IndexNumberWithLabel = edf_signal.IndexNumber + "." + edf_signal.Label;
